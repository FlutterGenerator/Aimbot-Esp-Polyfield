...
extern "C"
JNIEXPORT void JNICALL
Java_com_android_support_Menu_Init(JNIEnv *env, jobject thiz, jobject ctx, jobject title,
                                   jobject subtitle) {
    //Set sub title
    setText(env, title, OBFUSCATE("<b>😈 AIMBOT 😈</b>"));

    //Set sub title
    setText(env, subtitle, OBFUSCATE("ESP AND AIMBOT EDITION"));

    //Dialog Example
    //setDialog(ctx,env,OBFUSCATE("Title"),OBFUSCATE("Message Example"));

    //Toast Example
    Toast(env,ctx,OBFUSCATE("❤️ Welcome ❤️"),ToastLength::LENGTH_LONG);
